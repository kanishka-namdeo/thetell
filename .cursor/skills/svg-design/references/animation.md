...
**Never use `<animate>` tags inside JSX-rendered SVGs.** Vite's `EnvironmentPluginContainer.transform` crashes with `Cannot read properties of undefined (reading 'call')`. SMIL elements (`<animate>`, `<animateTransform>`) work but run on CPU with no GPU acceleration.
...
**Apply clip-path to individual child elements** (rect, path, circle), not to groups. For grouped clipping, use SVG `<clipPath>` with a `<path>` child, not CSS `clip-path`.
...
An SVG element with `transform="rotate(45)"` cannot have an independent CSS `transform: scale(0.5)` animation. CSS `transform` and SVG `transform` occupy the same property slot. The CSS value replaces the SVG attribute, displacing the element entirely.
...
- `<g>` (wrapped in a group)
- `<path>` (bare element, no wrapper)
- `<circle>` (bare element)
