...
Lucide convention prohibits `<g>`, `transform`, `<defs>`, `<use>`, `<mask>`, inline styles, and `<clipPath>`. Flatten everything to bare shape/path elements with styling on the root `<svg>`. Follow these constraints when building icon sets. For standalone icons or logos, you can be more flexible.
