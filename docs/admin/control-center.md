...
The page displays 6 stages in pipeline order. Each stage shows a status indicator, key metrics, and a manual trigger button.

```
Sources → Enrichment → Discovery → Analysis → Correlation → Articles
```
...
**What it does**: Runs AI analysis on pending signals. Each signal is analyzed by both agent personas (Analyst and Gossip Girl) to extract facts, sentiment, themes, and generate articles.
...
**What it does**: Cross-signal analysis that clusters themes by embedding similarity, detects convergence across source types, tracks theme momentum, and generates strategic inferences when 3+ signals from 2+ source types agree.

**Trigger**: "Run Correlation" — sends `correlation/manual.trigger` event.

**Metrics**: Themes detected, inferences generated.

**Automated schedule**: No dedicated cron — correlation runs as part of the post-analysis pipeline. Use the manual trigger after a large batch of new analyses to update theme clusters and inferences.

### Stage 6: Articles

**What it does**: Generates news-style articles from analysis results. Articles are created in the agent's voice (Analyst or Gossip Girl) and queued for publication.

**Trigger**: "Generate Missing Articles" — triggers article generation for analyzed signals that don't yet have articles.

**Metrics**: Articles generated (published), articles pending.

**Automated schedule**: Article generation runs as part of the analysis pipeline for each new signal. Use the manual trigger to backfill articles for signals that were analyzed but didn't get articles (e.g., due to transient errors).

## Troubleshooting

### Stage shows "idle" but should have run recently

Check the Inngest dashboard (`/dashboard/admin/operations/observability`) for failed jobs. Common causes:
...
### Correlation not generating inferences

Inferences require 3+ signals from 2+ source types agreeing on a theme. If you have few signals or they cover unrelated topics, correlation will detect themes but not generate inferences. This is expected behavior — not a bug.
...
| System (`/dashboard/admin/operations`) | System health, job monitoring, observability |
| Pipeline View (`/dashboard/admin/pipelines`) | Per-company pipeline status and scraper grid |
| Intelligence (`/dashboard/admin/intelligence`) | Themes and inferences generated by correlation |
