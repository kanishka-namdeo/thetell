...
async function main() {
  console.log("=== RE-ANALYZE FAILED SIGNALS ===\n");
  ...
  console.log(`Found ${failedSignals.length} FAILED signals\n`);

  let successCount = 0;
  let failCount = 0;

  for (const signal of failedSignals) {
    console.log(`\n[${successCount + failCount + 1}/${failedSignals.length}] ${signal.title.slice(0, 60)}`);
