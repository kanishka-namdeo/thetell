...
export interface AdminInferenceData {
  id: string;
  title: string;
  confidence: number;
  status: string;
...
interface AdminInferencesViewProps {
  initialInferences: AdminInferenceData[];
}

export function AdminInferencesView({ initialInferences }: AdminInferencesViewProps) {
  ...
  const handleResolve = async (
    id: string,
    newStatus: "CONFIRMED" | "REFUTED" | "RESOLVED",
  ) => {
    setResolving(id);
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to update inference",
      );
    } finally {
      setResolving(null);
    }
