...
export default function ModerationSettingsPage() {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/v1/admin/moderation/settings", { signal: controller.signal });
      if (!response.ok) {
      ...
      setAutoApproveSources(
        Array.isArray(data.autoApproveSources) ? data.autoApproveSources.join(", ") : ""
      );
      ...
      <div className="space-y-6">
        <div className="border-b-2 border-foreground pb-4">
          ...
          <p className="text-sm text-muted-foreground font-body mt-1">
            Configure moderation workflow and automation rules
          </p>
        </div>
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              Automatically approve content with confidence above this threshold (0.0-1.0)
            </p>
            <Input
              type="number"
              min="0"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              Automatically approve content from these source types (comma-separated)
            </p>
            <Input
      ...
      <Card className="border-2 border-foreground">
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <label className="text-sm font-medium">Notify on New Content</label>
              <p className="text-xs text-muted-foreground">
