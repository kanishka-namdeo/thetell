...
const components: Components = {
  code({ className, children, ...props }) {
    ...
    if (isInline) {
      return (
        <code
          className="rounded-sm bg-muted px-1.5 py-0.5 text-sm font-mono"
          {...props}
        >
          {children}
        </code>
  ...
  pre({ children }) {
    return (
      <pre className="rounded-lg bg-muted p-4 overflow-x-auto text-sm my-3">
        {children}
      </pre>
    );
  },
...
export function MarkdownContent({ content }: MarkdownContentProps) {
  return (
    <div className="prose-compact text-sm text-foreground">
      <ReactMarkdown
