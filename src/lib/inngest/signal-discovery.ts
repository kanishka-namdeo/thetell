...
async function resolveCompanyIds(companyIds: string[] | "all"): Promise<Array<{ id: string; name: string; ticker: string | null; websiteUrl: string | null; sector: string | null; description: string | null; industry: string | null }>> {
  if (companyIds === "all") {
    return prisma.company.findMany({
      select: { id: true, name: true, ticker: true, websiteUrl: true, sector: true, description: true, industry: true },
    });
  }
  return prisma.company.findMany({
...
async function processFeedItem(
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(companyId)) {
    log.error("Invalid companyId format (expected UUID, got slug)", { companyId, itemUrl: item.link });
    if (runId) await addPipelineLog(runId, "error", `Invalid company ID format: ${companyId}`, { url: item.link });
    return;
  }
  ...
  try {
    ...
    try {
      ...
      log.error("Failed to trigger analysis", { signalId: signal.id, error: String(error) });
    }
  } catch (embedError) {
